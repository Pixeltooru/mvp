...
try:
    ...
    with open(PUBLIC_KEY_PATH, 'wb') as f:
        f.write(pem_public)
    logger.info("Новые JWT ключи сохранены в файлы")

VPAID_PRIVATE_KEY_PATH = os.getenv('VPAID_PRIVATE_KEY_PATH', '/var/mvp/vpaid_private.pem')
VPAID_PUBLIC_KEY_PATH = os.getenv('VPAID_PUBLIC_KEY_PATH', '/var/mvp/vpaid_public.pem')
try:
    with open(VPAID_PRIVATE_KEY_PATH, 'rb') as f:
        vpaid_pem_private = f.read()
    vpaid_private_key = serialization.load_pem_private_key(vpaid_pem_private, password=None, backend=default_backend())
    with open(VPAID_PUBLIC_KEY_PATH, 'rb') as f:
        vpaid_pem_public = f.read()
    vpaid_public_key = serialization.load_pem_public_key(vpaid_pem_public, backend=default_backend())
    logger.info("VPAID ключи успешно загружены из файлов")
except FileNotFoundError:
    logger.warning("VPAID ключи не найдены, генерируем новые и сохраняем")
    vpaid_private_key = rsa.generate_private_key(
        public_exponent=65537,
        key_size=2048,
        backend=default_backend()
    )
    vpaid_public_key = vpaid_private_key.public_key()
    vpaid_pem_private = vpaid_private_key.private_bytes(
        encoding=serialization.Encoding.PEM,
        format=serialization.PrivateFormat.PKCS8,
        encryption_algorithm=serialization.NoEncryption()
    )
    vpaid_pem_public = vpaid_public_key.public_bytes(
        encoding=serialization.Encoding.PEM,
        format=serialization.PublicFormat.SubjectPublicKeyInfo
    )
    with open(VPAID_PRIVATE_KEY_PATH, 'wb') as f:
        f.write(vpaid_pem_private)
    with open(VPAID_PUBLIC_KEY_PATH, 'wb') as f:
        f.write(vpaid_pem_public)
    logger.info("Новые VPAID ключи сохранены в файлы")
...
# Не используйте открыте stun/turn сервера в конечном продукте!
ICE_SERVERS = [
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        response.headers['Content-Security-Policy'] = (
            ...
            "img-src 'self' data:; "
            "connect-src 'self' https://example.com:8088 https://mlo.example.com https://mlo.example.com:8088 wss://example.com:8089 wss://mlo.example.com:8089;"
        )
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://mlo.example.com", "https://example.com", "file://"],
...
async def create_engine_with_retry(url):
    # Упрощенная и более совместимая инициализация async-движка без параметров пула,
    # т.к. разные async-драйверы по-разному поддерживают эти опции
...
class User(Base):
    ...
    e2e_key_updated = Column(DateTime, nullable=True)
    last_activity = Column(DateTime, nullable=False, default=datetime.utcnow)
    # Удалено: subscriptions
...
async def init_db():
    try:
        async with engine.begin() as conn:
            ...
            tables_to_create = ['users', 'e2e_keys', 'subscriptions']
...
async def create_access_token(data: dict):
    ...
    encoded = jwt.encode(to_encode, pem_private, algorithm=JWT_ALGORITHM)
    if len(encoded) > 4096:
...
async def verify_token(token: str):
    ...
    try:
        payload = jwt.decode(token, pem_public, algorithms=[JWT_ALGORITHM], audience=JWT_AUDIENCE)
        jti = payload.get("jti")
        if jti and await redis_client.exists(f"jwt_blacklist:{jti}"):
            logger.warning("Токен отозван")
            raise HTTPException(status_code=401, detail="Токен отозван. Пожалуйста, войдите заново.")
        logger.debug(f"Успешно декодирован токен: sub={payload.get('sub')}")
        return payload
    except jwt.ExpiredSignatureError:
        logger.warning("Токен истек")
        raise HTTPException(status_code=401, detail="Токен истек. Пожалуйста, войдите заново.")
    except jwt.InvalidSignatureError as e:
        logger.error(f"Ошибка проверки подписи токена: {str(e)}")
        raise HTTPException(status_code=401, detail="Недопустимый токен: ошибка проверки подписи")
    except jwt.InvalidAudienceError:
        logger.warning("Неверная аудитория в токене")
        raise HTTPException(status_code=401, detail="Недопустимый токен: неверная аудитория")
...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security), db: AsyncSession = Depends(get_db)):
    try:
        ...
        async with db.begin():
            stmt = select(User).filter(User.unique_id == int(payload.get("sub")))
        ...
        raise HTTPException(status_code=500, detail="Ошибка аутентификации. Попробуйте заново.")
...
async def check_ws_rate_limit(user_id: str, client_ip: str):
    ...
    if count == 1:
        await redis_client.expire(ws_rate_key, WS_RATE_LIMIT_PERIOD)
...
async def check_failed_logins(client_ip: str):
    ...
    count = await redis_client.get(failed_key)
    count = int(count) if count else 0
    if count >= FAILED_LOGIN_LIMIT:
        await redis_client.set(block_key, 1, ex=FAILED_LOGIN_BLOCK_PERIOD)
...
async def increment_failed_login(client_ip: str):
    failed_key = f"failed_login:{client_ip}"
    count = await redis_client.incr(failed_key)
    if count == 1:
...
async def login(req: LoginRequest, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    async with db.begin():
        try:
            ...
            token = await create_access_token({"sub": str(user.unique_id)})
            csrf_token = secrets.token_urlsafe(32)
            csrf_key = f"csrf:{user.unique_id}"
            await redis_client.set(csrf_key, csrf_token, ex=ACCESS_TOKEN_EXPIRE_MINUTES * 60)
...
async def get_public_key():
    ...
    return {"public_key": pem_public.decode()}
...
@app.get("/vpaid_public_key")
async def get_vpaid_public_key():
    logger.debug("Запрос публичного VPAID ключа (без авторизации)")
    return {"vpaid_public_key": vpaid_pem_public.decode()}

@app.get("/csrf_token")
async def get_csrf_token(current_user: User = Depends(get_current_user)):
    csrf_key = f"csrf:{current_user.unique_id}"
    csrf_token = await redis_client.get(csrf_key)
    if not csrf_token:
        csrf_token = secrets.token_urlsafe(32)
        await redis_client.set(csrf_key, csrf_token, ex=ACCESS_TOKEN_EXPIRE_MINUTES * 60)
...
async def register_push(payload: dict, current_user: User = Depends(get_current_user)):
    try:
        ...
        if not push_token:
            raise HTTPException(status_code=400, detail="Отсутствует push_token")
        value = {
            "endpoint": push_token,
        ...
        await redis_client.set(key, json.dumps(value), ex=30*24*3600)
...
async def subscribe(req: SubscribeRequest, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    csrf_key = f"csrf:{current_user.unique_id}"
    stored_csrf = await redis_client.get(csrf_key)
    ...
    async with db.begin():
        try:
            ...
            await redis_client.rpush(message_key, notification)
...
async def confirm_subscribe(req: SubscribeRequest, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    async with db.begin():
        try:
            ...
            await redis_client.delete(f"secret_sub:{req.target_id}:{current_user.unique_id}")
            await redis_client.delete(f"secret_sub:{current_user.unique_id}:{req.target_id}")
            ...
            for uid_str in user_ids.split(','):
                ...
                if uid_str in active_connections:
                    ...
                    if la and (now - la < timedelta(minutes=5)):
                        ...
                        base_status = 'closed'
                typing_to_me = await redis_client.exists(f"typing:{uid_str}:{current_user.unique_id}")
...
async def websocket_handler(websocket, path: str):
    ...
    # Разрешаем origins для Electron (file:// и null), а также боевые домены
    allowed_origins = ['https://pixeltoo.ru', 'https://mlo.pixeltoo.ru', 'file://', 'null']
    if origin not in allowed_origins:
        logger.warning(f"Неверный origin для WS: {origin}, remote_addr={remote_addr}")
        await websocket.close(code=1008, reason="Неверный origin")
        return
    await check_ws_connections_per_ip(client_ip)
    logger.debug(f"Попытка подключения WebSocket: path={path}, remote_addr={remote_addr}, user-agent={mask_sensitive(user_agent)}")
    try:
        # Попробуем извлечь токен из заголовка Authorization
        token = websocket.request_headers.get('authorization', '').replace('Bearer ', '')
        if not token:
            # Если токен не найден в заголовке, проверяем строку запроса
            query_params = parse_qs(path.lstrip('/?'))
            token = query_params.get('token', [None])[0]
            if not token:
                logger.error(f"WebSocket: Отсутствует токен авторизации, remote_addr={remote_addr}, user-agent={mask_sensitive(user_agent)}")
                await websocket.close(code=1008, reason="Отсутствует токен авторизации")
                return
        try:
            payload = await verify_token(token)
            logger.debug(f"WebSocket: Токен успешно проверен, sub={payload.get('sub')}, remote_addr={remote_addr}")
        except HTTPException as e:
            logger.error(f"WebSocket: Ошибка токена: {e.detail}, remote_addr={remote_addr}, user-agent={mask_sensitive(user_agent)}")
            await websocket.close(code=1008, reason=f"Ошибка токена: {e.detail}")
            return
        ...
        async with AsyncSessionLocal() as db:
            async with db.begin():
                ...
                if not user:
                    ...
                    return
                # Для true E2E: public key уже в user, клиент использует для шифрования
                user.last_activity = datetime.utcnow()
                db.add(user)
        ...
        if messages:
            for msg in messages:
                try:
                    await websocket.send(msg)  # Отправляем raw (зашифрованные клиентом или метаданные)
                    logger.debug(f"Отправлено сохраненное сообщение для {user_id}, remote_addr={remote_addr}")
                except Exception as e:
        ...
        try:
            while True:
                try:
                    ...
                    logger.debug(f"WebSocket: Получены данные от {user_id}: {data_enc[:50]}..., remote_addr={remote_addr}")
                    if not isinstance(data_enc, str):
                        logger.warning(f"WebSocket: Получены нестроковые данные от {user_id}, remote_addr={remote_addr}")
                        continue
                    if len(data_enc) > 2**20:
                        logger.warning(f"WebSocket: Слишком длинное сообщение от {user_id}, remote_addr={remote_addr}")
                        continue
                    # Для true E2E: все сообщения - JSON blobs (plain для ping/pong, encrypted для сообщений)
                    # Сначала пробуем парсить как plain JSON для ping
                    data = None
                    try:
                        ...
                        if data.get('type') == 'ping':
                            ...
                            for sub_uid in subs_unique_ids:
                                ...
                                typing_from_sub = await redis_client.exists(f"typing:{sub_uid}:{user_id}")
                            ...
                            pong_json = json.dumps(response)
                            await websocket.send(pong_json)
                            logger.debug(f"Отправлен pong с статусами контактов для {user_id}")
                            continue
                    except json.JSONDecodeError:
                        pass
                    # Теперь парсим как base64-encoded JSON (для сообщений, звонков, typing и т.д.)
                    if data is None:
                        try:
                            ...
                            logger.warning(f"WebSocket: Неверный формат данных от {user_id}: {str(e)}, remote_addr={remote_addr}")
                            continue
                    if 'type' not in data:
                        logger.warning(f"WebSocket: Неверный формат данных от {user_id}: {data}, remote_addr={remote_addr}")
                        continue
                    ...
                    if data['type'] not in ['ping', 'get_history']:
                        if not target_uid_str or not re.match(r'^\d{8}$', target_uid_str):
                            logger.warning(f"WebSocket: Неверный target_id от {user_id}: {target_uid_str}, remote_addr={remote_addr}")
                            continue
                    
                    # Проверяем подписку (кроме ping/get_history)
                    if data['type'] not in ['ping', 'get_history']:
                        async with AsyncSessionLocal() as db_check:
                            async with db_check.begin():
                                ...
                                target_pk = result_target.scalar_one_or_none()
                                if not target_pk:
                                    logger.warning(f"WebSocket: Целевой пользователь {target_uid_str} не найден, remote_addr={remote_addr}")
                                    continue
                                stmt = select(Subscription).filter(Subscription.user_id == user.id, Subscription.target_id == target_pk, Subscription.status == 'accepted')
                                result = await db_check.execute(stmt)
                                if not result.scalar_one_or_none():
                                    logger.warning(f"WebSocket: {user_id} не подписан на {target_uid_str}, remote_addr={remote_addr}")
                                    continue
                    
                    # Опционально: верифицируем подпись
                    if 'signature' in data:
                        message_to_verify = json.dumps({k: v for k, v in data.items() if k != 'signature'}).encode()
                        # Get public key
                        async with AsyncSessionLocal() as db_sig:
                        ...
                        if public_pem and not verify_signature(message_to_verify, base64.b64decode(data['signature']), public_pem):
                            logger.warning(f"WebSocket: Неверная подпись от {user_id}")
                            continue
                    ...
                    if data['type'] == 'get_history':
                        try:
                            ...
                            if history_key:
                                ...
                                if total:
                                    # Возвращаем пустые элементы для совместимости UI (контент зашифрован)
                                    messages = []
                            await websocket.send(json.dumps({"type": "history", "messages": messages}))
                        except Exception as e:
                            logger.warning(f"WebSocket: Ошибка получения истории для {user_id}: {str(e)}")
                        continue

                    # Для typing
                    if data['type'] == 'typing_start':
                        await redis_client.set(f"typing:{user_id}:{target_uid_str}", 1, ex=10)
                    elif data['type'] == 'typing_stop':
                        await redis_client.delete(f"typing:{user_id}:{target_uid_str}")
                    
                    # Для call_start/end: обрабатываем метаданные
                    call_pending_key = f"call_pending:{user_id}:{target_uid_str}"
                    if data['type'] == 'call_end':
                        await redis_client.delete(call_pending_key)
                        logger.debug(f"Удален pending звонок для {user_id}->{target_uid_str}")
                        continue
                    elif data['type'] == 'call_start':
                        if await redis_client.exists(call_pending_key):
                            logger.debug(f"Не сохранен call_start, т.к. есть call_end для {user_id}->{target_uid_str}")
                            continue
                        await redis_client.set(call_pending_key, data_enc, ex=3600)
                    
                    # Храним историю для сообщений
                    message_types_for_history = ['ping', 'call_start', 'call_end', 'typing_start', 'typing_stop']
                    if data.get('type') not in message_types_for_history:
                        history_key_from = f"history:{user_id}:{target_uid_str}"
                        history_key_to = f"history:{target_uid_str}:{user_id}"
                        await redis_client.rpush(history_key_from, data_enc)
                        await redis_client.rpush(history_key_to, data_enc)
                        await redis_client.expire(history_key_from, HISTORY_TTL)
                        await redis_client.expire(history_key_to, HISTORY_TTL)
                        logger.debug(f"Сохранено зашифрованное сообщение в историю для {user_id} <-> {target_uid_str}")
                    
                    # Forwarding зашифрованного сообщения
                    message_key = f"ws_message:{target_uid_str}"
                    await redis_client.rpush(message_key, data_enc)
                    await redis_client.expire(message_key, 86400)
                    if target_uid_str in active_connections:
                        try:
                            await active_connections[target_uid_str]['websocket'].send(data_enc)
                            logger.debug(f"WebSocket: Отправлено зашифрованное сообщение от {user_id} к {target_uid_str}, remote_addr={remote_addr}")
                        except Exception as e:
                            logger.warning(f"WebSocket: Не удалось отправить сообщение {target_uid_str}: {str(e)}, remote_addr={remote_addr}")
                    else:
                        logger.warning(f"WebSocket: Пользователь {target_uid_str} не в сети, remote_addr={remote_addr}")
                except asyncio.TimeoutError:
                    logger.debug(f"WebSocket: Таймаут для {user_id}, remote_addr={remote_addr}")
                    continue
                except websockets.exceptions.ConnectionClosed as e:
                    logger.debug(f"WebSocket: Соединение {user_id} закрыто: code={e.code}, reason={e.reason}, remote_addr={remote_addr}")
                    break
                except ssl.SSLError as e:
                    logger.error(f"WebSocket: Ошибка SSL для {user_id}: {str(e)}, remote_addr={remote_addr}")
                    break
                except Exception as e:
                    logger.error(f"WebSocket: Ошибка обработки сообщения для {user_id}: {str(e)}, remote_addr={remote_addr}")
                    continue
        finally:
        ...
        logger.error(f"WebSocket: HTTP ошибка: {e.detail}, remote_addr={remote_addr}")
        await websocket.close(code=1008, reason=e.detail)
    except Exception as e:
        logger.error(f"WebSocket: Критическая ошибка авторизации: {str(e)}, remote_addr={remote_addr}, user-agent={mask_sensitive(user_agent)}")
